...
--]]
	...
	if playerObj:getInventory():contains("xiuchegongjv") then
		self.context:addOption(getText("IGUI_xiuligongjvxiuli"), playerObj, ISVehicleMechanics.onCheatRepairPart1, part)
	end

	
	
	
	--self.chr:getInventory():AddItem("BIase.zombiemeat")
...
function ISVehicleMechanics.onCheatRepairPart1(playerObj, part)
	sendClientCommand(playerObj, "vehicle", "repairPart", { vehicle = part:getVehicle():getId(), part = part:getId() })
	playerObj:getInventory():Remove("xiuchegongjv")
    playerObj:Say(getText("IGUI_xiuligongjvxiulisay"))
end
